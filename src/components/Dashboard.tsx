...
const Dashboard: React.FC<DashboardProps> = ({ user }) => {
  ...
  const { operations, loading: loadingOperations, error: operationsError } = useBettingOperations();
  ...
  const updateDashboardData = () => {
    ...
    // Calcula métricas totais para o período filtrado
    const totalBets = sortedOperations.length;
    const totalBetAmount = sortedOperations.reduce((sum, op) => sum + (Number(op.bet_amount) || 0), 0);
    const totalProfit = sortedOperations.reduce((sum, op) => sum + (Number(op.profit) || 0), 0);
    const totalResult = sortedOperations.reduce((sum, op) => sum + (Number(op.result) || 0), 0);
    
    // Calcula ROI baseado no total investido
    const roi = totalBetAmount > 0 ? (totalProfit / totalBetAmount) * 100 : 0;
    
    // Calcula média por conta
    const uniqueAccountsTotal = new Set(sortedOperations.map(op => op.id)).size;
    const profitPerAccount = uniqueAccountsTotal > 0 ? totalProfit / uniqueAccountsTotal : 0;

    // Agrega os dados para o gráfico (últimos 7 dias)
    ...
    for (let i = 6; i >= 0; i--) {
      ...
      const dayBetAmount = dayOperations.reduce((sum, op) => sum + (Number(op.bet_amount) || 0), 0);
      const dayResult = dayOperations.reduce((sum, op) => sum + (Number(op.result) || 0), 0);
      const dayProfit = dayOperations.reduce((sum, op) => sum + (Number(op.profit) || 0), 0);
      
      last7DaysData.push({
        x: dateString,
        y: dayProfit,
        bets: dayBets,
        accounts: uniqueAccountsTotal,
    ...
    setDashboardData({
      todayBets: totalBets,
      todayBetsAverage: totalBets > 0 ? totalBetAmount / totalBets : 0,
      todayProfit: totalProfit,
      roi: roi,
      profitPerAccount: profitPerAccount,
      accountsUsed: uniqueAccountsTotal,
  ...
  useEffect(() => {
    ...
    // Se houver um parâmetro bank, selecione essa banca
    if (bankParam) {
  ...
  const applyFilters = (operations) => {
    return operations.filter(op => {
      ...
      // Ajusta as datas para incluir 30 dias antes e depois
